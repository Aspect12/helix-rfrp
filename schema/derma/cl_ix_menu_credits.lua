...
vgui.Register("ixREDCreditsLogo", PANEL, "Panel")
...
function PANEL:Init()
	...
	self.name:SetColor(color_black)
...
vgui.Register("ixREDCreditsNametag", PANEL, "Panel")
...
function PANEL:Init()
	...
	self.nametag = self:Add("ixREDCreditsNametag")

	self.tags = self:Add("DLabel")
	self.tags:SetFont("ixMenuButtonFont")
	self.tags:SetColor(color_black)
...
vgui.Register("ixREDCreditsRow", PANEL, "Panel")
...
function PANEL:AddLeft(name, steamid)
	local nametag = self:Add("ixREDCreditsNametag")
...
vgui.Register("ixREDCreditsSpecials", PANEL, "Panel")

PANEL = {}

function PANEL:Init()
	self:Add("ixREDCreditsLogo")

	local link = self:Add("DLabel", self)
	link:SetFont("ixMenuMiniFont")
	link:SetTextColor(Color(50, 50, 50, 255))
	link:SetText(url)
	...
	for _, v in ipairs(CREDITS) do
		local row = self:Add("ixREDCreditsRow")
		row:SetName(v[1])
		row:SetAvatar(v[2])
		row:SetTags(v[3])
		row:SizeToContents()
	...
	local specialList = self:Add("ixREDCreditsSpecials")
	...
	for _, v in ipairs(MISC) do
		...
		description:SetColor(color_black)
